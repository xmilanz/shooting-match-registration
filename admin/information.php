            ...
            <div id='collapseTwo' class='accordion-collapse collapse' data-bs-parent='#accordionInformation'>
                <div class='accordion-body'>
                    <fieldset class='<?= (($line['bulkId'] > 0) ? 'border p-3 my-3 rounded' : '') ?>'>
                        <legend class='float-none w-auto px-2 h6 <?= (($line['bulkId'] > 0) ? '' : 'd-none') ?>'>Registrace závodníka</legend>
                        <div class='row pb-3'>
                            <div class='col-md-5 <?= ($line['Disciplina'] == 'VYRAZENO' ? 'd-none' : '') ?>'>
                                ...
                                <select class="form-select" name=Disciplina required>
                                    <option value="<?= htmlspecialchars($line['Disciplina'], ENT_QUOTES, 'UTF-8'); ?>"><?= $nazev_discipliny ?></option>
                                    <?php
                                    $query = mysqli_query($conn, "SELECT * from $table_disciplines ORDER BY Value");
                            ...
                            <div class='col-md-5 <?= (empty($line['DisciplinaReg']) ? 'd-none' : '') ?>'>
                                ...
                                <input readonly class='bg-light text-muted form-control' value='<?= htmlspecialchars($line['DisciplinaReg'], ENT_QUOTES, 'UTF-8') ?>'>
                            </div>
                            <div class='col-md-5 <?= $zavodCisloZbraneClass ?>'>
                                <label class='form-label'>Číslo zbraně</label>
                                <input readonly class='bg-light text-muted form-control' value='<?= htmlspecialchars($line['CisloZbrane'], ENT_QUOTES, 'UTF-8') ?>'>
                            </div>
                        </div>
                        <div class='row pb-3'>
                            <div class='col-md-5'>
                            ...
                            <div class='col-md-3 <?= (empty($match_data['Payment_before']) ? '' : 'd-none') ?>'>
                            ...
                            <div class='col-md-4 <?= (empty($match_data['Payment_before']) ? '' : 'd-none') ?>'>
                                <label class='form-label'>Zaplatit</label>
                                <input readonly class='bg-light text-dark form-control' value="<?= htmlspecialchars($line['CastkaZaplatit'], ENT_QUOTES, 'UTF-8') ?> <?= $match_data['Banka_ucet_MENA'] ?>">
                            </div>
                            <div class='col-md-4 pt-4 <?php echo (($line['Staff'] == "PAY") && ($match_data['Payment_before'] == "on")) ? '' : 'd-none' ?>'>
                                ...
                                <table class='table table-hover table-bordered disciplines'>
                                    ...
                                    <tbody>
                                        ...
                                        SELECT 
                                            ...
                                            Castka
                                                FROM $table
                                                WHERE bulkId = $line[bulkId] AND Disciplina != 'VYRAZENO'
                                                ORDER BY Cislo
                                            ");
                                        $bulkStmt->execute();
                                        ...
                                        while ($bulkLine = mysqli_fetch_array($result)) {
                                        ?>
                                            <tr>
