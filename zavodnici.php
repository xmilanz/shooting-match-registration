...
if (!$match_data['Payment_before']) {
    ...
    $stmt = $conn->prepare("
        SELECT count(Disciplina) as comp FROM $table    
        where UPPER(`Disciplina`) != 'VYRAZENO' and Zaplaceno='on'
    ");
    $stmt->execute();
    $result = $stmt->get_result();
    $stmt->close();
...
$stmt = $conn->prepare("
    ...
    where UPPER(`Disciplina`) != 'VYRAZENO' and Zaplaceno IS NULL
");
$stmt->execute();
$result = $stmt->get_result();
$stmt->close();
...
if (!$match_data['Payment_before']) {
    ...
    $stmt = $conn->prepare("
        SELECT Disciplina,Count(Prijmeni) as Count FROM $table    
        where UPPER(`Disciplina`) != 'VYRAZENO' and Zaplaceno='on' GROUP BY Disciplina HAVING count(Prijmeni)>=1 ORDER BY Disciplina
    ");
}
$stmt->execute();
$result = $stmt->get_result();
